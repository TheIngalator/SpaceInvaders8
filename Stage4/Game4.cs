/***********************************************************************************************************************************
 * 
 * This program now has code which does the following:
 * 1. Spawns multiple aliens one at a time at intervals. Nine different aliens are stored in a 2D array and randomly selected.
 * 2. Handles shooting fire from the cannon. The cannon is fired each time the space button in pressed.
 * The program draws fire above the cannon and then makes the fire ball travel upwards. In the next program code will be written to
 * check for collisions with an alien. 
 * 
 * The following subroutines have been added to the program:
 * - fireCanon(): draws the fireball. The function is called when the space button is pressed
 * 
 * - spawnRandomSprite(): called at random intervals from the Update() method. SpawnRandomSprite uses the random function to select a random
 * number between 1-9 to select any one one of the 9 sprites. The update method has a timer which is checked at random intervals to call
 * the spawnRandomSprite() method.
 * 
 * - drawAliens() iterates through an arrayList which stores spawned sprites and changes each sprites y position at intervals controlled by
 * the timer in the mlUpdate method
 * 
 * 
 * 20/12/2022
 * 
...
namespace Stage4
{
    public class Game4 : Game
    {
        ...
        protected override void Update(GameTime gameTime)   //add your update logic here
        {
            ...
            int randomInterval = rnd.Next(5, 10);
            ...
            if ((timer > threshold / 4) && (cannonFired == true))   //using threshold/4 to increase speed of cannon fire
            {
                ...
                if (firePos.Y <= 0)
                {   // check for alien collision or fire reaching boundary
                    fireFactor = 0;
                    cannonFired = false;
                }
